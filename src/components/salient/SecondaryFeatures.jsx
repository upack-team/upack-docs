...
import { Container } from "@site/src/components/salient/Container";
import screenshotProject from "@site/src/images/screenshots/project.png";
import screenshotCost from "@site/src/images/screenshots/cost.png";
import screenshotHR from "@site/src/images/screenshots/hr.png";
...
const features = [
	{
		...
		description: "华炎项目管理解决方案帮助您跟踪和管理项目，里程碑，任务，已阻止的任务，逾期任务，时间，费用预算，并具有详细的报表功能。",
	...
	{
		name: "费控管理",
		summary: "实现从费控管理的全程数字化，打通组织内的业财数据，实现业务财务一体化。",
	...
	{
		name: "人事管理",
		summary: "员工的入转调离与考勤、薪酬等环节自动打通，让企业人事充分感受一站式的便利。",
...
function Feature({ feature, isActive, className, ...props }) {
	return (
		<div className={clsx(className, !isActive && "opacity-75 hover:opacity-100")} {...props}>
			...
			<h3 className={clsx("mt-6 text-base font-medium", isActive ? "text-blue-600" : "text-slate-600 dark:text-slate-400")}>{feature.name}</h3>
			<p className="mt-2 font-display text-xl text-slate-900 dark:text-slate-200">{feature.summary}</p>
				...
				<>
					...
					<Tab.Panels className="relative mt-20 overflow-hidden rounded-4xl bg-slate-200 px-14 py-16 xl:px-16">
						<div className="-mx-5 flex">
							{features.map((feature, featureIndex) => (
								<Tab.Panel static key={feature.name} className={clsx("px-5 transition duration-500 ease-in-out [&:not(:focus-visible)]:focus:outline-none", featureIndex !== selectedIndex && "opacity-60")} style={{ transform: `translateX(-${selectedIndex * 100}%)` }} aria-hidden={featureIndex !== selectedIndex}>
		...
		<section id="secondary-features" aria-label="Features for simplifying everyday business tasks" className="pt-20 pb-14 sm:pb-20 sm:pt-32 lg:pb-32">
			<Container>
				<div className="mx-auto max-w-4xl md:text-center">
					<h2 className="font-display text-4xl tracking-tight text-slate-900 sm:text-5xl dark:text-slate-100">无代码达到边界，华炎魔方推动您突破边界</h2>
					<p className="mt-4 text-lg tracking-tight text-slate-700 dark:text-slate-400">华炎魔方允许企业以非常灵活的方式创建和定制应用。通过其高度可编程的环境，企业可以解决复杂的业务需求，实现全面的数字化转型和自动化，从而真正推动业务增长和创新。华炎魔方可以支持多种企业应用场景，包括但不限于OA、CRM、HR、ERP、BI、IoT、大数据等。</p>
				</div>
				<FeaturesMobile />
				<FeaturesDesktop />
			</Container>
